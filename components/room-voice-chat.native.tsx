...
function VoiceControls() {
  ...
  const toggleSpeaker = () => { const next = !speaker; setSpeaker(next); InCallManager.setForceSpeakerphoneOn(next); };
  const connected = connectionState === ConnectionState.Connected;

  return <View style={styles.card}>
    <View style={styles.heading}><Text style={styles.title}>ROOM VOICE · SFU</Text><View style={styles.counter}><Text style={styles.counterText}>{Math.max(0, participants.length - 1)} CONNECTED</Text></View></View>
    <Text style={styles.status}>{connected ? "Room voice channel connected — all members can speak and listen" : `Voice ${String(connectionState).toLowerCase()}…`}</Text>
    <View style={styles.actions}>
      <Pressable disabled={busy} onPress={toggleMic} style={({ pressed }) => [styles.action, isMicrophoneEnabled && styles.actionActive, pressed && styles.pressed]}><Text style={styles.actionLabel}>{isMicrophoneEnabled ? "MIC ON" : "MIC OFF"}</Text></Pressable>
      <Pressable onPress={toggleSpeaker} style={({ pressed }) => [styles.action, speaker && styles.actionActive, pressed && styles.pressed]}><Text style={styles.actionLabel}>{speaker ? "SPEAKER ON" : "SPEAKER OFF"}</Text></Pressable>
    </View>
    <Text style={styles.hint}>Dedicated room voice uses LiveKit SFU and Opus with low-latency routing, echo cancellation, noise suppression and automatic gain control.</Text>
  </View>;
}

export const RoomVoiceChat = forwardRef<RoomVoiceChatHandle, Props>(function RoomVoiceChat({ mediaToken: suppliedToken, socket, memberId }: Props, ref) {
  const [mediaToken, setMediaToken] = useState<MediaToken | null | undefined>(suppliedToken);
  useImperativeHandle(ref, () => ({ setMicrophoneEnabled: async () => undefined, setSpeakerEnabled: async (enabled: boolean) => { InCallManager.setForceSpeakerphoneOn(enabled); } }), []);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!cancelled) setMediaToken(envelope.result?.data?.json ?? { configured: false, message: "تعذر تجهيز قناة الصوت." });
      } catch { if (!cancelled) setMediaToken({ configured: false, message: "تعذر الاتصال بخدمة الصوت الجماعي." }); }
    };
    void load(); return () => { cancelled = true; };
  }, [suppliedToken, socket, memberId]);

  if (!mediaToken?.configured || !mediaToken.url || !mediaToken.token) return <View style={styles.card}><Text style={styles.title}>ROOM VOICE</Text><Text style={styles.status}>{mediaToken?.message ?? "تعذر تجهيز قناة الصوت."}</Text></View>;
  return <LiveKitRoom serverUrl={mediaToken.url} token={mediaToken.token} connect audio={true} video={false} options={{ adaptiveStream: true, publishDefaults: { audioPreset: { maxBitrate: 96000 }, dtx: false, red: true, forceStereo: false } }}><VoiceControls /></LiveKitRoom>;
});
