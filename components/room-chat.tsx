...
export function RoomChat({ socket, title = "دردشة الغرفة" }: { socket: Socket | null; title?: string }) {
  ...
  return (
    <View style={styles.card}>
      ...
      <View style={styles.messages}>
        {messages.length ? messages.slice(-4).map((message) => <Text key={message.id} style={styles.message}><Text style={styles.sender}>{message.displayName}: </Text>{message.text}</Text>) : <Text style={styles.empty}>{socket?.connected ? "اكتب رسالة للاعب الآخر." : "اربط قناة الغرفة لتفعيل الدردشة."}</Text>}
      </View>
      <View style={styles.composer}>
        <TextInput value={draft} onChangeText={setDraft} editable={Boolean(socket?.connected)} placeholder="اكتب رسالة…" placeholderTextColor="#71839A" style={styles.input} textAlign="right" returnKeyType="send" onSubmitEditing={send} />
        <Pressable onPress={send} disabled={!socket?.connected || !draft.trim()} style={({ pressed }) => [styles.send, (!socket?.connected || !draft.trim() || pressed) && styles.sendDisabled]}><Text style={styles.sendText}>إرسال</Text></Pressable>
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  message: { color: "#D5E1EB", fontSize: 12, lineHeight: 19, textAlign: "right" },
  sender: { color: "#62C2EB", fontWeight: "900" },
  empty: { color: "#879AAF", fontSize: 12, textAlign: "right" },
