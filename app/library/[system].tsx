...
export default function EmulatorLibraryScreen() {
  ...
  const chooseGame = async (settingsMode = false) => {
    ...
    try {
      ...
      await MoudieEmulatorModule.launchNativeGame(system, asset.uri, asset.name, { orientation, aspectRatio, settingsMode });
    } catch (error) {
    ...
    <ScreenContainer className="px-5" edges={["top", "bottom", "left", "right"]} containerClassName="bg-background">
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.settingsPanel}>
          ...
          <Text style={styles.settingsHint}>Open controller settings once for portrait and once for landscape. Drag or pinch each button independently, then choose SAVE & PLAY.</Text>
          <Pressable onPress={() => chooseGame(true)} disabled={loading} style={({ pressed }) => [styles.settingsLaunch, (pressed || loading) && styles.launchPressed]}><MaterialCommunityIcons name="tune-variant" size={20} color="#CBEFFF" /><Text style={styles.settingsLaunchText}>CHOOSE FILE & CONFIGURE {orientation.toUpperCase()} CONTROLS</Text></Pressable>
        </View>
        <Pressable onPress={() => chooseGame(false)} disabled={loading} style={({ pressed }) => [styles.launch, { backgroundColor: meta.color }, (pressed || loading) && styles.launchPressed]}>{loading ? <ActivityIndicator color="#09121D" /> : <MaterialCommunityIcons name="folder-open-outline" size={23} color="#09121D" />}<Text style={styles.launchText}>{loading ? "PREPARING GAME…" : capability?.downloadable && !capability.available ? "CHOOSE GAME & DOWNLOAD CORE" : "CHOOSE GAME & START"}</Text></Pressable>
