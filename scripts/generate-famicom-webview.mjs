...
(function(){
  ...
  function inspectRom(buffer){var bytes=new Uint8Array(buffer);if(bytes.length<16||bytes[0]!==78||bytes[1]!==69||bytes[2]!==83||bytes[3]!==26){return {error:'This file does not contain a valid iNES header.'};}var mapper=(bytes[6]>>4)|(bytes[7]&240);if((bytes[7]&12)===8){return {error:'NES 2.0 files are not supported by the embedded player yet.'};}if(!supportedMappers[mapper]){return {error:'Mapper '+mapper+' is not supported by the embedded Famicom player yet.'};}return {mapper:mapper};}
