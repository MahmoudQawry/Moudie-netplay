 ...
 *
 * Room capacity is intentionally separate from controller capacity: a room may
 * contain spectators, while the emulator must never be told to expose more
 * simultaneous controller ports than its core supports.
 */
object NativeCoreCatalog {
  ...
  private val definitions = listOf(
    Definition(
      system = "nes", title = "Famicom / NES", coreName = "FCEUmm",
      libraryNames = listOf("fceumm_libretro_android.so"), extensions = setOf("nes", "fds", "zip"),
      netplay = "retroarch", maxControllerSlots = 2, maxRoomPlayers = 2, maxRoomSpectators = 6,
      profile = EmulatorControlProfiles.FAMICOM, systemDirectory = "moudie-nes/system",
    ),
    Definition(
      system = "ps1", title = "PlayStation 1", coreName = "PCSX-ReARMed",
      libraryNames = listOf("pcsx_rearmed_libretro_android.so", "libpcsx_rearmed_libretro_android.so"),
      extensions = setOf("bin", "cue", "iso", "chd", "pbp"), netplay = "retroarch",
      maxControllerSlots = 8, maxRoomPlayers = 6, maxRoomSpectators = 4,
      profile = EmulatorControlProfiles.PS1, systemDirectory = "moudie-ps1/system",
    ),
    Definition(
      system = "psp", title = "PlayStation Portable", coreName = "PPSSPP",
      libraryNames = listOf("ppsspp_libretro_android.so"), extensions = setOf("iso", "cso", "chd", "pbp"),
      netplay = "psp-network", maxControllerSlots = 4, maxRoomPlayers = 6, maxRoomSpectators = 4,
      profile = EmulatorControlProfiles.PSP, systemDirectory = "moudie-psp/system",
    ),
    Definition(
      system = "sega", title = "Sega Genesis / Mega Drive", coreName = "Genesis Plus GX",
      libraryNames = listOf("genesis_plus_gx_libretro_android.so"), extensions = setOf("bin", "md", "gen", "smd", "sms", "gg", "zip"),
      netplay = "retroarch", maxControllerSlots = 4, maxRoomPlayers = 6, maxRoomSpectators = 4,
      profile = EmulatorControlProfiles.SEGA, systemDirectory = "moudie-sega/system",
    ),
    Definition(
      system = "arcade", title = "Arcade", coreName = "MAME Arcade",
      libraryNames = listOf("mamearcade_libretro_android.so"), extensions = setOf("zip", "7z", "chd"),
      netplay = "retroarch", maxControllerSlots = 4, maxRoomPlayers = 6, maxRoomSpectators = 4,
      profile = EmulatorControlProfiles.ARCADE, systemDirectory = "moudie-arcade/system",
  ...
  /** Downloads an explicitly configured optional core on a worker thread. */
  fun downloadCore(context: Context, definition: Definition): File? {
    ...
    return runCatching {
      val connection = (URL(sourceUrl).openConnection() as HttpsURLConnection).apply {
        connectTimeout = 15_000; readTimeout = 120_000; instanceFollowRedirects = true; useCaches = false; requestMethod = "GET"
      }
      try {
        ...
        ZipInputStream(connection.inputStream.buffered()).use { archive ->
          var entry = archive.nextEntry; var extracted = false
          while (entry != null) {
            if (!entry.isDirectory && definition.libraryNames.any { entry.name.endsWith(it) }) {
              FileOutputStream(temporary).use { output -> archive.copyTo(output); output.fd.sync() }
      ...
      } finally { connection.disconnect(); if (temporary.exists()) temporary.delete() }
    }.getOrNull()
  }
  ...
  private fun extractPackagedCore(context: Context, definition: Definition, targetDirectory: File): File? {
    ...
    return runCatching {
      ZipFile(context.applicationInfo.sourceDir).use { apk ->
        val entry = Build.SUPPORTED_ABIS.asSequence().flatMap { abi -> definition.libraryNames.asSequence().map { "lib/$abi/$it" } }
  ...
  private fun isUsableCore(file: File): Boolean = file.isFile && file.length() >= 4L && runCatching {
    file.inputStream().use { input -> val header = ByteArray(4); input.read(header) == 4 && header.contentEquals(byteArrayOf(0x7f, 0x45, 0x4c, 0x46)) }
  }.getOrDefault(false)
}
